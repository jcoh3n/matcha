  ...
  (
    ...
    if (variant === "discovery") {
      return (
        <Card
          ref={ref}
          className={cn(
            "rounded-2xl max-w-4xl mx-auto bg-white shadow-soft animate-scale-in overflow-hidden",
            className
          )}
          {...props}
        >
          <div className="flex flex-col md:flex-row h-[480px] md:h-[520px]">
            ...
            <div className="flex flex-col justify-center w-full md:w-1/2 px-8 md:px-12 font-montserrat bg-white">
              ...
              <p className="mt-6 flex items-center gap-2 text-sm md:text-base text-muted-foreground font-medium">
                <MapPin className="w-4 h-4 text-primary" />
                {formatDistance(user.distance, user.location)}
              </p>
              {(onLike || onPass) && (
                <div className="mt-10 flex items-center gap-8">
                  {onPass && (
                    <button
                      onClick={handlePass}
                      className="w-16 h-16 rounded-full bg-secondary hover:bg-secondary-muted text-muted-foreground hover:text-foreground shadow-sm flex items-center justify-center transition active:scale-95"
                      aria-label="Pass"
                    >
                      <X className="w-7 h-7" />
                    </button>
    ...
    if (variant === "compact") {
      return (
        <Card
          ref={ref}
          className={cn("rounded-2xl overflow-hidden w-56", className)}
          {...props}
        >
    ...
    return (
      <Card
        ref={ref}
        className={cn(
          "rounded-2xl overflow-hidden interactive-card",
          className,
          "h-72 relative"
        )}
        {...props}
